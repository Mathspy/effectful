...
fn hello_world() {
    ...
    // crates/crate_name/tests/folder because file!() does not interact very
    // nicely with env::current_dir()
    //
    // current_dir() == "~/Coding/effectful/crates/parser"
    // file!() == "crates/parser/tests/hello_world.rs"
    //
    // So if this was in crate_root/tests we'd need to not go up two directories because the values
    // will be more sane
    //
